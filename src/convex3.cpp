...
string detect_cases(vector <double> line ,vector <City*> idx_hull, int index){
    ...
    if(position_next ==1 && position_prev == -1 )
    {
        // next is above, prev is below, select next as pivot. 
        //cout<<"next"<<endl ;
        return "next"; 
    }
    else if (position_next ==1 && position_prev == 1 )
    {
        // both are above ; 
...
vector <City*> combiner(vector <City*>&left_hull ,vector<City*>&right_hull) {
    ...
    int left_most_right= 0;
    int right_most_left= left_hull.size()-1; 
    ...
    while(main_controller[0] || main_controller[1]){
        ...
        line = create_line(left_hull[left_index_tangent],right_hull[right_index_tangent]); 
        for(int i=0;i<line.size();i++){
            cout<<line[i]<<" ";
        }
        cout<<"Coefficients of the line printed "<<endl ; 
    ...
    while(main_controller[0] || main_controller[1]){
        // create line's equation: 
        lower_tangent=create_line(left_hull[left_index_tangent%left_hull.size()],right_hull[right_index_tangent]); 
        // first check the right halve
        // utilize the detect case method ! 
        if(loop_controller_right_hull)
        {
    ...
    lower_tangent_vector[0]= left_index_tangent ;
    ...
    int index=0 ; 
    // BURDAAAAAAAAAAAAAAAAAYIMMMMMMMMMMMMMMM
    // @todo 
    cout<<"INFO OF THE TANGENT INDEX:"<<endl ;
